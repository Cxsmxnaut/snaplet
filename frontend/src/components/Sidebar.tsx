...
export const Sidebar = ({ activeTab, onTabChange, isCollapsed, onToggleCollapse }: SidebarProps) => {
  ...
  return (
    <aside className={cn(
      ...
      <div className={cn("flex mb-7 text-on-surface-variant transition-all duration-300 ease-in-out", isCollapsed ? 'px-0 items-center justify-center' : 'px-2.5 items-center gap-3')}>
        <button
        ...
        {navItems.map((item) => (
          <button
            ...
            className={cn(
              ...
              isCollapsed ? "h-11 w-11 justify-center mx-auto px-0" : "w-full max-w-[150px]",
              activeTab === item.id 
      ...
      <div className={cn("mt-7 h-px bg-[#e9edf5] transition-[margin] duration-300 ease-in-out", isCollapsed ? 'mx-3' : 'mx-2')} />
      ...
      <div className="pt-5 mt-auto">
        ...
        >
          ...
          <span className={cn(
            "overflow-hidden whitespace-nowrap transition-all duration-300 ease-in-out",
            isCollapsed ? "max-w-0 opacity-0 -translate-x-1" : "max-w-20 opacity-100 translate-x-0"
          )}>
            Help
          </span>
        </button>
