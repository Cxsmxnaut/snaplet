...
create index if not exists product_events_created_at_idx
  on private.product_events (created_at desc);

create index if not exists product_events_user_created_at_idx
  on private.product_events (user_id, created_at desc);

create index if not exists product_events_name_created_at_idx
  on private.product_events (event_name, created_at desc);

alter table private.product_events enable row level security;

drop policy if exists "product_events_select_own" on private.product_events;
drop policy if exists "product_events_insert_own" on private.product_events;
drop policy if exists "product_events_insert_anon" on private.product_events;

create policy "product_events_select_own"
on private.product_events
for select
to authenticated
